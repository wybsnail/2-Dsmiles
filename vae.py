    ...
    def loadState(self):
        if os.path.isfile(self.state_fname):
            try:
                ...
                utils.logger.warning(f"Skip loading encoder checkpoint due to mismatch: {err}")
        else:
    ...
    def loadState(self):
        if os.path.isfile(self.state_fname):
            try:
                ...
                utils.logger.warning(f"Skip loading decoder checkpoint due to mismatch: {err}")
        else:
            utils.logger.info("state file is not found")
...
class VAE(object):
    ...
    def latent_space_quality(self, nSample, tokenizer=None, is_valid_fn=None):
        self.decoder.eval()
    ...
    def trainModel(self, dataloader, encoderOptimizer, decoderOptimizer, encoderScheduler, decoderScheduler, KLD_alpha, nepoch, tokenizer, printInterval, is_valid_fn=None):
