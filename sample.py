...
def main():
    ...
    if model_type == 'rnn':
        ...
        model = gan.GAN(**utils.config['gan_param'],
                        ...
                        device=device)
        model.generator.loadState()
    elif model_type == 'flow':
        model = flow.Flow(**utils.config['flow_param'], state_fname=utils.config['fname_flow_parameters'], device=device)
        model.loadState()
    elif model_type == 'diffusion':
        model = diffusion.Diffusion(**utils.config['diffusion_param'], state_fname=utils.config['fname_diffusion_parameters'], device=device)
        model.loadState()
    elif model_type == 'flow':
        model = flow.Flow(**utils.config['flow_param'], state_fname=utils.config['fname_flow_parameters'], device=device)
        model.loadState()
