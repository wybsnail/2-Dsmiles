...
class SmilesDataset(torch.utils.data.Dataset):
    def __init__(self, fname, tokenizer, maxLength):
        ...
        with open(fname, 'r') as f:
            # Skip FASTA headers (lines starting with '>') and blank lines to allow protein datasets
            self.data = [sm.strip() for sm in f.readlines() if sm.strip() and not sm.startswith('>')]
